...
let mytrackerslist = undefined
const getTrackers = () => {
  const https = require('https')
  const options = {
    hostname: 'cdn.jsdelivr.net',
    port: 443,
    path: '/gh/XIU2/TrackersListCollection@master/all.txt',
    method: 'GET'
  }
  const req = https.request(options, res => {
    // console.log('request_start');
    let trackerslist = defaultAnnounceList
      .map((arr) => arr[0])
    res.setEncoding('utf8');
    res.on('data', chunk => {
      chunk = chunk.split('\n\n')
      trackerslist.push(...chunk)
      // chunk.forEach(tracker => { trackerslist.push(tracker) });
    })
    res.on('end', () => {
      global.WEBTORRENT_ANNOUNCE = trackerslist
      mytrackerslist = trackerslist
      // electron.dialog.showMessageBox({
      //   type: 'info',
      //   buttons: [],
      //   title: 'trackerCheck',
      //   message: 'trackerNum: ' + trackerslist.length
      // })
    })
  })
